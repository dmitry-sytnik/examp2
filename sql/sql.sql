--
-- База данных: `ecommerce2`
--

-- Не выполняйте эту страницу в пакетном режиме!
-- Несколько хранимых процедур были переопределены.
-- Измените этот файл SQL и выполните его в пакетном режиме либо скопируйте и вставьте нужные команды.
...
--
-- Добавление данных в таблицу `general_coffees`
--

INSERT INTO `general_coffees` VALUES(1, 'Original Blend', 'Высококачественная смесь кофейных зерен средней обжарки, обладающая насыщенным цветом и тонким вкусом.', 'original_coffee.jpg');
INSERT INTO `general_coffees` VALUES(2, 'Dark Roast', 'Кофе сильной обжарки, обладающий полным вкусом и горьковатым послевкусием.', 'dark_roast.jpg');
INSERT INTO `general_coffees` VALUES(3, 'Kona', 'Кофе сорта Kona произрастает на горных плантациях Гавайских островов. Тонкий вкус и великолепная обжарка!', 'kona.jpg');
...
--
-- Добавление данных в таблицу 'non_coffee_categories`
--

INSERT INTO `non_coffee_categories` VALUES(1, 'Sladosti', 'Превосходный ассортимент сладостей. Предлагаются бискотти, пахлава, лимонное печенье и другие вкусности!', 'goodies.jpg');
INSERT INTO `non_coffee_categories` VALUES(2, 'Korziny', 'Подарочные корзины для любых случаев! Продаются вместе с кофе и другими товарами.', 'gift_basket.jpg');
INSERT INTO `non_coffee_categories` VALUES(3, 'Chaski', 'Великолепный выбор кружек, которые позволят вам наслаждаться кофе, чаем, горячим шоколадом и другими напитками.', '781426_32573620.jpg');
INSERT INTO `non_coffee_categories` VALUES(4, 'Knigi', 'Предлагаем книги о кофе, о кулинарии, а также книги Ларри Ульмана!', 'books.jpg');
...
--
-- Добавление данных в таблицу `non_coffee_products`
--

INSERT INTO `non_coffee_products` (`non_coffee_category_id`, `name`, `description`, `image`, `price`, `stock`, `date_created`) VALUES
(3, 'Кофейная чашка с цветком', 'Симпатичная кофейная чашка, украшенная цветком на белом фоне.', 'd9996aee5639209b3fb618b07e10a34b27baad12.jpg', 650, 100, NOW()),
(3, 'Чашка с красным драконом', 'Дизайнерская чашка, украшенная золотым драконом на красном фоне. Частично складывающаяся симпатичная ручка.', '847a1a3bef0fb5c2f2299b06dd63669000f5c6c4.jpg', 795, 4, NOW());
...
('goodies', 1, 500, '2014-08-16', '2014-10-31'),
('coffee', 7, 700, '2014-08-19', NULL),
('coffee', 9, 1300, '2014-08-19', '2014-08-26'),
('goodies', 2, 700, '2014-08-22', NULL),
('coffee', 8, 1300, '2014-08-22', '2014-10-31'),
('coffee', 10, 3000, '2014-08-22', '2014-11-30');
...
--
-- Добавление данных в таблицу `sizes`
--

INSERT INTO `sizes` VALUES(1, '50 грамм, проба');
INSERT INTO `sizes` VALUES(2, '200 грамм');
INSERT INTO `sizes` VALUES(3, '400 грамм');
INSERT INTO `sizes` VALUES(4, '800 грамм');
INSERT INTO `sizes` VALUES(5, '2 кг');
...
--
-- Добавление данных в таблицу `specific_coffees`
--

INSERT INTO `specific_coffees` (`general_coffee_id`, `size_id`, `caf_decaf`, `ground_whole`, `price`, `stock`, `date_created`) VALUES
(3, 1, 'обычный', 'молотый', 200, 20, NOW()),
(3, 2, 'обычный', 'молотый', 450, 30, NOW()),
(3, 2, 'без кофеина', 'молотый', 500, 20, NOW()),
(3, 3, 'обычный', 'молотый', 800, 50, NOW()),
(3, 3, 'без кофеина', 'молотый', 850, 20, NOW()),
(3, 3, 'обычный', 'в зернах', 750, 50, NOW()),
(3, 3, 'без кофеина', 'в зернах', 800, 20, NOW()),
(3, 4, 'обычный', 'в зернах', 1500, 30, NOW()),
(3, 4, 'без кофеина', 'в зернах', 1550, 15, NOW()),
(3, 5, 'обычный', 'в зернах', 3250, 5, NOW());
...
BEGIN
SELECT CONCAT("G", ncp.id) AS sku, c.quantity, ncc.category, ncp.name, ncp.price, ncp.stock, sales.price AS sale_price FROM carts AS c 
...
UNION 
SELECT CONCAT("C", sc.id), c.quantity, gc.category, CONCAT_WS(" - ", s.size, sc.caf_decaf, sc.ground_whole), sc.price, sc.stock, sales.price FROM carts AS c 
...
END$$
DELIMITER ;
-- Следует добавлять CHARACTER SET 'utf8' к параметрам CHAR и VARCHAR во избежание ошибок - Дм.С.
...
-- Процедура принимает идентификатор (ID) покупателя (cid), идентификатор (ID) корзины покупателя (uid), стоимость доставки (ship) и номер банковской карты (четыре последние цифры, cc). Также применяются два исходящих аргумента: total и oid. В блоке BEGIN определена локальная  переменная  под  названием subtotal.
DELIMITER $$
...
-- Затем добавляется запись  в таблицу orders и извлекается только что созданный идентификатор заказа, который присваивается аргументу oid. 
...
-- с помощью запроса INSERT ... SELECT UNION SELECT заполняется таблица order_contents.
	INSERT INTO order_contents (order_id, product_type, product_id, quantity, price_per) 
-- Конструкция IFNULL() выбирает первое значение, если оно не равно NULL, и второе значение в том случае, если первое значение равно NULL
		SELECT oid, c.product_type, c.product_id, c.quantity, IFNULL(sales.price, ncp.price) FROM carts AS c 
		INNER JOIN non_coffee_products AS ncp ON c.product_id=ncp.id 
		LEFT OUTER JOIN sales ON (sales.product_id=ncp.id AND sales.product_type='goodies' AND ((NOW() BETWEEN sales.start_date AND sales.end_date) OR (NOW() > sales.start_date AND sales.end_date IS NULL)) ) 
		WHERE c.product_type="goodies" AND c.user_session_id=uid 
		UNION 
		SELECT oid, c.product_type, c.product_id, c.quantity, IFNULL(sales.price, sc.price) FROM carts AS c 
		INNER JOIN specific_coffees AS sc ON c.product_id=sc.id 
		LEFT OUTER JOIN sales ON (sales.product_id=sc.id AND sales.product_type='coffee' AND ((NOW() BETWEEN sales.start_date AND sales.end_date) OR (NOW() > sales.start_date AND sales.end_date IS NULL)) ) 
		WHERE c.product_type="coffee" AND c.user_session_id=uid;
-- В последних трех строках кода вычисляется промежуточный итог для заказа, обновляется столбец total в таблице orders и присваивается значение общего итога переменной total.
-- Для  подсчета итоговой  суммы  выполняется  агрегирующий  запрос к таблице orders_contents для текущего заказа
	SELECT SUM(quantity*price_per) INTO subtotal FROM order_contents WHERE order_id=oid;
-- Теперь можно обновить таблицу orders
	UPDATE orders SET total = (subtotal + ship) WHERE id=oid;
-- Итоговую сумму присваивают исходящему аргументу
